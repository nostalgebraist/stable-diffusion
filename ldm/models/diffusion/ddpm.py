...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None:
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc, dict) or isinstance(xc, list) or isinstance(xc, tuple):
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                ...
                log["conditioning"] = xc
            elif self.cond_stage_key in ["caption_transcription"]:
                combined = [cc + '\n-------------------\n' + ct for cc, ct in zip(*batch[self.cond_stage_key])]
                xc = log_txt_as_img((x.shape[2], x.shape[3]), combined)
                log["conditioning"] = xc
    ...
    def get_multiple_conditioning_for_guidance(
        ...
        full_in = bs * [caption_drop_string]
        ...
        segments = torch.split(full_c, bs)
        c = segments[0]
        for j, uc_segment in enumerate(segments[1:]):
            uc.append(uc_segment)
